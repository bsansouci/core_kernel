...
(* All the types as exposed in the mli are defined in this [Types] module.  The rest of
   this file is simply overriding all the compare, and sexp functions to raise
   exceptions. *)
module Types = struct
...
module type Sexpable_binable_comparable = sig
  type 'a t = 'a [@@deriving compare, sexp]
end

(* Override all sexp, compare functions to raise exceptions *)
...
module Only_used_as_phantom_type0 (T : sig
  ...
  type t = T.t [@@deriving compare, sexp_poly]
end = struct
  module M = Only_used_as_phantom_type1 (T)
  type t = T.t M.t [@@deriving compare, sexp]
...
module Stable = struct
  module V1 = struct
    ...
    type nobody = Nobody.t [@@deriving compare, sexp]
    type me     = Me.t     [@@deriving compare, sexp]

    module Upper_bound = struct
      module M = Only_used_as_phantom_type1 (Types.Upper_bound)
      type 'a t = 'a Types.Upper_bound.t M.t [@@deriving compare, sexp]
      let __t_of_sexp__ = t_of_sexp
    end
  end

  module Export = struct
    type read       = V1.Read.          t [@@deriving compare, sexp]
    type write      = V1.Write.         t [@@deriving compare, sexp]
    type immutable  = V1.Immutable.     t [@@deriving compare, sexp]
    type read_write = V1.Read_write.    t [@@deriving compare, sexp]
    type 'a perms   = 'a V1.Upper_bound.t [@@deriving compare, sexp]
  end
end
